...
help:
	...
	@echo "  make migrations-prod-up      Apply pending PRODUCTION migrations under the mutation lock"
	...
	@echo "  make prod-rollback-workflow-test Run rollback workflow contract tests (CI-safe)"
	@echo "  make prod-rollback-schema-test Run rollback schema gate tests (CI-safe)"
	@echo "  make prod-rollback-critical-test Run rollback critical section tests (CI-safe)"
...
# The production migration. Separate from migrations-up on purpose: this one
# takes the production mutation lock by construction, so no operator can migrate
# production without it, and a conflict with a rollback refuses instead of
# queueing.
migrations-prod-up:
	pnpm migrations:prod:up
...
# Proves the manual production rollback workflow is a rollback and nothing else:
# no build, no migration, no candidate deploy, no Secret, PVC or DNS mutation,
# and no path that derives its target or falls back to the other slot. Offline:
# parses the workflow, touches no cluster.
prod-rollback-workflow-test:
	pnpm prod:rollback-workflow:test

# Proves the rollback schema gate refuses a rollback across a contract-phase
# migration and every history it cannot walk. Offline: builds throwaway git
# repositories, touches no database and no cluster.
prod-rollback-schema-test:
	pnpm prod:rollback-schema:test

# Runs the rollback critical section end to end against the fake cluster: the
# mutation lock, the ledger read inside it, the schema gate, rollback.sh, and
# the signal and BUSY paths. Offline: no cluster, no database.
prod-rollback-critical-test:
	pnpm prod:rollback-critical:test

# Proves the production runner's pre-job guard authorises exactly the two
# contexts it lists -- the production deploy and the production rollback, each on
# main and dispatched by hand -- and refuses every other repository, workflow,
# ref and event. Offline: runs the guard itself, touches no runner, no cluster.
