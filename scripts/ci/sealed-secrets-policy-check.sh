...
while IFS= read -r file; do
  case "$file" in
    infra/k8s/secrets/templates/*.template.yaml|infra/k8s/base/secrets.example.yaml)
      if grep -E 'POSTGRES_PASSWORD:[[:space:]]*"?[^" ]+' "$ROOT_DIR/$file" | grep -Ev 'REPLACE_ME|replace-me' >/dev/null; then
        echo "POSTGRES_PASSWORD in $file must remain a placeholder." >&2
        exit 1
      fi
      ;;
    *.yaml|*.yml)
      if grep -E 'POSTGRES_PASSWORD:[[:space:]]*"?[^" ]+' "$ROOT_DIR/$file" | grep -Ev 'REPLACE_ME|replace-me|\$\{[A-Z0-9_]+\}' >/dev/null; then
        echo "POSTGRES_PASSWORD appears in non-template YAML with a non-placeholder value: $file" >&2
        exit 1
      fi
      ;;
  esac
