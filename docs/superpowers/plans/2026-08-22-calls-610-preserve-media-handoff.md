...
```ts
  const runWriteMediaIntent = (callId: string, captured: OwnerLease, intent: MediaIntent): boolean => {
    if (closed || !isBoundedString(callId)) return false;
    if (captured.tabId !== tabId) return false;
    if (!lease || lease.callId !== callId || lease.tabId !== tabId || lease.epoch !== captured.epoch) {
      return false;
    }
    try {
      ...
      const revision = previous && previous.ownershipEpoch === captured.epoch ? previous.revision + 1 : 1;
...
```ts
  const wrappedToggleMicrophone = useCallback(async (): Promise<boolean | undefined> => {
    ...
    const camera = confirmedIntentRef.current?.callId === callId
  ...
  const wrappedToggleCamera = useCallback(async (): Promise<boolean | undefined> => {
    ...
    if (result === undefined || !lease || !callId) return result;
    const microphone = confirmedIntentRef.current?.callId === callId
