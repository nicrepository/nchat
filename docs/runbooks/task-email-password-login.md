...
- **Traefik / reverse proxy must overwrite or sanitize both inbound
  `X-Forwarded-For` and `X-Real-IP`** before forwarding to auth-service;
  auth-service must not be directly reachable from the internet when trusted
...
| `AUTH_TRUSTED_PROXY_CIDRS`                  | Comma-separated CIDRs of trusted reverse proxies (e.g. `10.0.0.0/8`); when the request RemoteAddr is in this list, the leftmost `X-Forwarded-For` entry is used for rate-limiting if it is a valid IP (canonicalized via `parsed.String()`); `X-Real-IP` is the fallback if valid; otherwise `RemoteAddr` is used. Empty = use RemoteAddr only (default, safe for single-instance). |
