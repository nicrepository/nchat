...
Para validar uma mudança, identifique os arquivos web próprios da branch com `git diff --name-only origin/develop...HEAD` e os arquivos ainda não rastreados com `git status --short`, então execute. A comparação de três pontas parte do merge-base e evita incluir mudanças que existem apenas em `origin/develop`:
