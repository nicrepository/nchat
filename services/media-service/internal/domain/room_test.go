...
func TestRoomNameDerivesCanonicalNamesWithoutCollision(t *testing.T) {
	channelRoom, err := RoomName(ResourceKindChannel, roomTestID)
	if err != nil {
		t.Fatalf("channel room: %v", err)
	}
	dmRoom, err := RoomName(ResourceKindDM, roomTestID)
	if err != nil {
		t.Fatalf("dm room: %v", err)
	}

	if channelRoom != "channel:aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa" {
		t.Fatalf("unexpected channel room %q", channelRoom)
	}
	if dmRoom != "dm:aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa" {
...
func TestRoomNameRejectsUnknownKindAndInvalidUUID(t *testing.T) {
	for _, input := range []struct {
		...
		if _, err := RoomName(input.kind, input.id); err == nil {
