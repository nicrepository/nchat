...
func sidebarRouter(v *httpapi.TokenValidator, svc *stubSidebarProvider) http.Handler {
	return httpapi.NewRouter(sidebarTestConfig(), nil, v, allowAllSessionValidator{}, httpapi.NewSidebarHandler(svc), httpapi.NewMessageHandler(nil, nil))
}
...
func TestSidebarHandler_NilService_Returns503(t *testing.T) {
	...
	router := httpapi.NewRouter(sidebarTestConfig(), nil, v, allowAllSessionValidator{}, httpapi.NewSidebarHandler(nil), httpapi.NewMessageHandler(nil, nil))
