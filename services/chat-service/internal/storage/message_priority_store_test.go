...
// contract: before issue #824's acknowledgement flag and issue #825's
// persistent-notifications flag, and after the conversation event pair. Derived
...
func priorityColumnIndex() int { return len(messageCols()) - 3 }
...
func expectCreateWithPriority(mock pgxmock.PgxPoolIface, priority string, rows *pgxmock.Rows) {
	...
	// $23 is the priority this test is about. $24-$27 are issue #824's
	// acknowledgement flag and bound and issue #825's persistent-notifications
	// flag and reminder interval, all matched loosely because none of them is
	// what is under test here and pinning them would make this fixture fail for
	// the wrong reason.
