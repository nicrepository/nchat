...
type WorkspaceSettingsStore interface {
	...
	UpdateMaxUploadBytes(ctx context.Context, workspaceID, userID string, maxBytes int64) (domain.Workspace, error)
}
...
func (s *PGXWorkspaceStore) GetWorkspaceByID(ctx context.Context, id string) (domain.Workspace, error) {
	...
	err := s.pool.QueryRow(ctx, `
		SELECT id, slug, name, status, message_rate_limit_per_minute, max_upload_bytes,
		       created_at, updated_at
		FROM chat.workspaces
		WHERE id = $1`,
		id,
	).Scan(&w.ID, &w.Slug, &w.Name, (*string)(&w.Status), &w.MessageRateLimitPerMinute,
		&w.MaxUploadBytes, &w.CreatedAt, &w.UpdatedAt)
	if err != nil {
...
// UpdateMaxUploadBytes sets the RF-32 attachment size policy for a workspace.
//
// The join against chat.workspace_members is the authorization, not a lookup —
// identical in shape and intent to UpdateMessageRateLimit above: the row is only
// updated when userID is an active owner/admin *of that same workspace*, so a
// caller aiming the endpoint at a workspace they do not administer changes
// nothing and gets ErrNoRows. The handler checks the same thing before calling;
// this is the backstop that holds even if that check is ever removed, and it is
// what makes the write and the authorization a single atomic statement rather
// than a check followed by a racy update.
//
// maxBytes is expected to be pre-validated against the domain bounds; the CHECK
// constraint from migration 000020 rejects anything outside them, which surfaces
// here as a wrapped error rather than a silent clamp.
func (s *PGXWorkspaceStore) UpdateMaxUploadBytes(ctx context.Context, workspaceID, userID string, maxBytes int64) (domain.Workspace, error) {
	var w domain.Workspace
	err := s.pool.QueryRow(ctx, `
		UPDATE chat.workspaces w
		SET max_upload_bytes = $3, updated_at = now()
		FROM chat.workspace_members wm
		WHERE w.id = $1 AND w.status = 'active'
		  AND wm.workspace_id = w.id AND wm.user_id = $2
		  AND wm.status = 'active' AND wm.role IN ('owner', 'admin')
		RETURNING w.id::text, w.slug, w.name, w.status,
		          w.max_upload_bytes, w.created_at, w.updated_at`,
		workspaceID, userID, maxBytes,
	).Scan(&w.ID, &w.Slug, &w.Name, (*string)(&w.Status), &w.MaxUploadBytes, &w.CreatedAt, &w.UpdatedAt)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return domain.Workspace{}, domain.ErrForbidden
		}
		return domain.Workspace{}, fmt.Errorf("update workspace max upload bytes: %w", err)
	}
	return w, nil
}
