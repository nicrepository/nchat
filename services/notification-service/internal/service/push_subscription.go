...
type PushSubscriptionStore interface {
	...
	RecordDelivery(ctx context.Context, subscriptionID string, generation int64,
		result domain.DeliveryResult) (bool, error)
}
...
// of one subscription, and reports whether it applied.
//
...
// comes back false. That is expected in a system where a send and its answer are
// not simultaneous, and the caller has nothing to do about it.
//
...
// backoff all belong to the worker that will call this; none of them is here.
