...
func TestOIDCRoutesRegisteredOnRouter(t *testing.T) {
	...
	router := httpapi.NewRouter(cfg, platformlog.New("auth-service", "test"), nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, httpapi.AllowAllBootstrapAttemptsForTest{}, &fakeOIDCManager{loginErr: domain.ErrOIDCMisconfigured})
	rec := httptest.NewRecorder()
...
func TestOIDCRouterDisabledRoutesReturn404BeforeRateLimit(t *testing.T) {
	...
	router := httpapi.NewRouter(cfg, platformlog.New("auth-service", "test"), nil, nil, nil, nil, nil, nil, nil, nil, nil, nil, httpapi.AllowAllBootstrapAttemptsForTest{})
