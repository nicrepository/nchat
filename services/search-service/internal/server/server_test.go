...
func TestHealthzContract(t *testing.T) {
	...
	if body.Data.Service != "search-service" {
		t.Fatalf("expected service search-service, got %q", body.Data.Service)
	}
...
func TestVersionRouteStillWorks(t *testing.T) {
	...
	if body.Data["service"] != "search-service" || body.Data["version"] != "0.0.0" || body.Data["commit"] != "dev" {
...
func TestMethodAndNotFoundBehavior(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "post healthz", method: http.MethodPost, path: "/healthz", want: http.StatusMethodNotAllowed},
		{name: "post readyz", method: http.MethodPost, path: "/readyz", want: http.StatusMethodNotAllowed},
