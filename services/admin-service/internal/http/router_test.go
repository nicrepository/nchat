...
func TestHealthzContract(t *testing.T) {
	router := NewRouter(testConfig(), platformlog.New("admin-service", "test"))
	...
	if body.Data.Service != "admin-service" {
		t.Fatalf("expected service admin-service, got %q", body.Data.Service)
	}
...
func TestVersionRouteStillWorks(t *testing.T) {
	...
	if body.Data["service"] != "admin-service" || body.Data["version"] != "0.0.0" || body.Data["commit"] != "dev" {
...
func testConfig() config.Config {
	return config.Config{ServiceName: "admin-service", Env: "test", Port: 8085, ReadHeaderTimeoutSeconds: 5}
}
