...
// ── Unread/read grouping (Discord-style navigation) ──────────────────────────

describe("ChatSidebar — unread/read grouping", () => {
  const channel = (id: string, overrides: Partial<Channel> = {}): Channel => ({
    id,
    name: id,
    type: "public",
  ...
  const optionNamesIn = (name: string) =>
    within(section(name))
      .queryAllByRole("option")
      .map((option) => option.getAttribute("aria-label"));
  const subgroupLabelsIn = (name: string) =>
    within(section(name))
  ...
  it("splits a mixed channel list into Não lidas / Lidas, each still newest-first", () => {
    renderState(
      [
        channel("lido-recente", { lastMessageAt: "2026-08-04T12:00:00Z" }),
        channel("nao-lido-antigo", { unreadCount: 1, lastMessageAt: "2026-08-01T00:00:00Z" }),
        channel("nao-lido-novo", { unreadCount: 3, lastMessageAt: "2026-08-04T15:00:00Z" }),
        channel("lido-antigo", { lastMessageAt: "2026-08-02T00:00:00Z" }),
      ],
      [],
    );

    expect(subgroupLabelsIn("Canais")).toEqual(["Não lidas", "Lidas"]);
    expect(optionNamesIn("Canais")).toEqual([
      "Canal nao-lido-novo",
      "Canal nao-lido-antigo",
      "Canal lido-recente",
      "Canal lido-antigo",
    ]);
  });

  it("treats a mention-only conversation as unread for grouping purposes", () => {
    renderState(
      [],
      [
        dm("lido", "1:1", { lastMessageAt: "2026-08-04T12:00:00Z" }),
        dm("mencionado", "1:1", { hasMentionUnread: true, lastMessageAt: "2026-08-01T00:00:00Z" }),
      ],
    );

    expect(subgroupLabelsIn("Mensagens diretas")).toEqual(["Não lidas", "Lidas"]);
    expect(optionNamesIn("Mensagens diretas")).toEqual([
      "Mensagem direta com mencionado",
      "Mensagem direta com lido",
    ]);
  });

  it("renders a single ungrouped list, with no subheading, when everything is read", () => {
    renderState(
      [channel("a", { lastMessageAt: "2026-08-04T10:00:00Z" }), channel("b")],
      [dm("c", "1:1")],
    );

    expect(subgroupLabelsIn("Canais")).toEqual([]);
    expect(subgroupLabelsIn("Mensagens diretas")).toEqual([]);
  });

  it("renders a single ungrouped list, with no subheading, when everything is unread", () => {
    renderState(
      [channel("a", { unreadCount: 1 }), channel("b", { unreadCount: 2 })],
      [dm("c", "1:1", { unreadCount: 1 })],
    );
  ...
  it("re-groups a conversation from Não lidas to Lidas once its unread count clears", () => {
    const { rerender } = render(
      <MemoryRouter initialEntries={["/chat"]}>
        <ChatSidebar
          state={readyState(
            [
              channel("lido", { lastMessageAt: "2026-08-04T09:00:00Z" }),
              channel("geral", { unreadCount: 2, lastMessageAt: "2026-08-04T12:00:00Z" }),
            ],
            [],
          )}
          retry={() => {}}
    ...
    expect(subgroupLabelsIn("Canais")).toEqual(["Não lidas", "Lidas"]);
    expect(
      within(section("Canais")).getByRole("heading", { level: 3, name: "Não lidas" }),
    ).toBeInTheDocument();
      ...
      <MemoryRouter initialEntries={["/chat"]}>
        <ChatSidebar
          state={readyState(
            [
              channel("lido", { lastMessageAt: "2026-08-04T09:00:00Z" }),
              channel("geral", { unreadCount: 0, lastMessageAt: "2026-08-04T12:00:00Z" }),
            ],
            [],
          )}
          retry={() => {}}
    ...
    expect(subgroupLabelsIn("Canais")).toEqual([]);
    expect(optionNamesIn("Canais")).toEqual(["Canal geral", "Canal lido"]);
  });
});
